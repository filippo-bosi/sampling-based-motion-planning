...
class RRTGraph:
    ...
    def near(self, n, gamma=200, d=2, dmax=100):
        # qnew is at index n in node list
        # ((n nodes in the tree, not n+1 since qnew is not yet part of the tree))?
        q_primes = []

        k = gamma*math.pow(math.log(n+1)/(n+1),1/d)
        for i in range(0, n):
            if self.distance(i, n) <= k:
                q_primes.append(i)
        return q_primes
    def isFree(self):
    ...
    def RRT_star(self):
        X_new, Y_new, Parent_new = self.extend()
        #self.extend()
        q_new = self.number_of_nodes()-1
        # After the extend function I will get the q_new (last non-colliding node) at index n_new
        # It means I have already checked for possibles collisions and extended until the last node
        # print("Q new:")
        # print(q_new, X_new[-1], Y_new[-1])
        print("Parent:")
        print(Parent_new[-1])
        # OK now RRT_star begins
        # Get a set of tree configurations around q_new
        Qnear = self.near(q_new)
        print("Q near set:")
        print(Qnear)
        # find the node with minimum cost to go to q_new from q_start
        qmin = self.choose_parents(Qnear, q_new)
        print("qmin: ")
        print(qmin)
        if qmin is not None:
            self.connect(qmin, q_new)
        else:
            self.add_edge(Parent_new[-1], q_new)
        return self.x, self.y, self.parent

    def choose_parents(self, Qnear, q_new):
        # Compute the cost to q_new (at node index n_new) through a neighbor in Qnear
        # initialize the minimum cost with the value that it would have with RRT path
        minCost = self.cost(q_new)
        print("minCost:")
        print(minCost)
        q_min = None
        self.remove_edge(q_new)
        print("Node removed")
        for q_near in Qnear:
            self.add_edge(q_near, q_new)
            # cost(q_near) is the total cost to reach q_new from q_near
            if self.cost(q_near) + self.distance(q_near,q_new) < minCost :
                minCost = self.cost(q_near) + self.distance(q_near,q_new)
                q_min = q_near
            # remove edge to allow for the computation of different costs
            self.remove_edge(q_new)
        return q_min
